...
export function ProductDetailClient({
  ...
  const [selectedLength, setSelectedLength] = useState<Length>(Length.REGULAR);

  // Note
  const [note, setNote] = useState("");
  const [activeImage, setActiveImage] = useState(0);
  const [activeGuide, setActiveGuide] = useState<"size" | "length" | null>(
    null
  );
    ...
    <div className="bg-white dark:bg-black min-h-screen text-black dark:text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-12">
        ...
        <nav className="flex items-center gap-2 text-[10px] uppercase tracking-[0.2em] font-bold text-neutral-400 mb-8 lg:mb-12 overflow-x-auto whitespace-nowrap pb-2">
          ...
          <Link
            href={`/shop?category=${(product.category as any)?.name}`}
                  ...
                  {activeGuide === "size" ? (
                    ...
                    <div className="space-y-6">
                      ...
                      <table className="w-full text-center border-collapse text-xs">
                        <thead className="bg-black text-white dark:bg-white dark:text-black uppercase font-bold tracking-wider">
                          <tr>
                            <th className="p-3 border border-neutral-700 text-left">
                              Category
                            </th>
                        ...
                        <tbody>
                          {lengthStandards.map((row, i) => (
                            ...
                            >
                              <td className="p-3 border border-neutral-200 dark:border-neutral-700 font-bold text-left">
                                {row.part}
                              </td>
