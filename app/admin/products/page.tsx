...
export default function ProductsPage() {
    ...
    const fetchProducts = useCallback(async () => {
        ...
        try {
            ...
            if (selectedCollection !== 'all') params.append('collectionId', selectedCollection);
        ...
        <div className="space-y-10 pb-10 max-w-[1600px] mx-auto">
            ...
            <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
                ...
                <div className="flex items-center gap-3 w-full md:w-auto">
                    <Select value={selectedCategory} onValueChange={setSelectedCategory}>
                        ...
                        <SelectContent>
                            <SelectItem value="all" className="text-[10px] font-bold tracking-wider uppercase">All Categories</SelectItem>
                            {categories.map(cat => (
                                <SelectItem key={cat.id} value={cat.id} className="text-[10px] font-bold tracking-wider uppercase">{cat.name}</SelectItem>
                            ))}
                        </SelectContent>
                    </Select>
                    ...
                    <Select value={selectedCollection} onValueChange={setSelectedCollection}>
                        ...
                        <SelectContent>
                            <SelectItem value="all" className="text-[10px] font-bold tracking-wider uppercase">All Collections</SelectItem>
                            {collections.map(coll => (
                                <SelectItem key={coll.id} value={coll.id} className="text-[10px] font-bold tracking-wider uppercase">{coll.name}</SelectItem>
                            ))}
                        </SelectContent>
                    </Select>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="border-b border-neutral-100 dark:border-neutral-900 bg-neutral-50/50 dark:bg-neutral-900/10">
                                ...
                                <th className="px-6 py-5 text-[10px] font-black uppercase tracking-[0.15em] text-neutral-400">Product</th>
                                <th className="px-6 py-5 text-[10px] font-black uppercase tracking-[0.15em] text-neutral-400">Category</th>
                                <th className="px-6 py-5 text-[10px] font-black uppercase tracking-[0.15em] text-neutral-400">Collections</th>
                                <th className="px-6 py-5 text-[10px] font-black uppercase tracking-[0.15em] text-neutral-400 text-right">Selling Price</th>
                                <th className="px-8 py-5 text-right text-[10px] font-black uppercase tracking-[0.15em] text-neutral-400">Actions</th>
                            </tr>
                        </thead>
                            ...
                            {products.length === 0 && !isLoading ? (
                                <tr>
                                    <td colSpan={6} className="px-8 py-20 text-center">
                                        ...
                                        <p className="text-[10px] font-black uppercase tracking-[0.2em] text-neutral-400">No Collective Items Found</p>
                                    </td>
                                </tr>
                                ...
                                <tr key={product.id} className="group hover:bg-neutral-50/30 dark:hover:bg-neutral-900/5 transition-colors">
                                    <td className="px-8 py-6">
                                        <div className="flex items-center gap-3">
                                            ...
                                            />
                                            <span className={`text-[8px] font-black uppercase tracking-widest ${product.isActive ? 'text-emerald-500 shadow-emerald-500/20' : 'text-neutral-300'}`}>
                                    ...
                                    <td className="px-6 py-6">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div className="min-w-0 max-w-[200px]">
                                                <p className="text-[11px] font-bold text-black dark:text-white uppercase tracking-tight truncate">{product.name}</p>
                                                <p className="text-[9px] font-bold text-neutral-400 mt-0.5 tracking-tight uppercase truncate">{product.slug}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-6 py-6">
                                        <Badge variant="outline" className="text-[9px] font-black uppercase tracking-widest border-neutral-200 dark:border-neutral-800 bg-neutral-50/50 dark:bg-neutral-900/50 text-neutral-500">
                                    ...
                                    <td className="px-6 py-6 text-right">
                                        <p className="text-[11px] font-black text-black dark:text-white uppercase tracking-wider">GH₵ {product.price}</p>
                                        {product.discountId && (
                                            <p className="text-[8px] font-black text-emerald-500 mt-0.5 uppercase tracking-widest">Discount Applied</p>
                                        )}
                                    </td>
                                    <td className="px-8 py-6 text-right">
                                        <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <button
                                                ...
                                                title="View details"
                                            >
                                                {isLoadingPreview ? <Loader2 className="w-4 h-4 animate-spin text-neutral-400" /> : <Eye className="w-4 h-4 text-neutral-400" />}
                                            </button>
                                            <Link href={`/admin/products/upload?id=${product.id}`} className="p-2 hover:bg-neutral-100 dark:hover:bg-neutral-900 rounded-md transition-colors" title="Edit product">
            ...
            <AlertDialog open={!!deleteId} onOpenChange={(open) => !open && !isDeleting && setDeleteId(null)}>
                <AlertDialogContent className="max-w-md rounded-2xl border-neutral-100 dark:border-neutral-800 p-8">
                    <AlertDialogHeader className="space-y-4">
                        <AlertDialogTitle className="font-serif text-2xl font-bold tracking-tight">Permanent Deletion?</AlertDialogTitle>
                        <AlertDialogDescription className="text-neutral-500 font-light leading-relaxed">
                            This action is absolute. The product and all its variants, images, and pricing history will be permanently erased from the Awuraba archives.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter className="mt-8 sm:space-x-4">
                        <AlertDialogCancel className="border-neutral-200 dark:border-neutral-800 text-[10px] uppercase font-black tracking-widest rounded-full h-11 px-8">Return</AlertDialogCancel>
                        <AlertDialogAction
                            onClick={(e) => {
                                e.preventDefault();
                                handleDelete();
                        ...
                        >
                            {isDeleting ? <Loader2 className="w-4 h-4 animate-spin" /> : "Confirm Erase"}
                        </AlertDialogAction>
                    </AlertDialogFooter>
                </AlertDialogContent>
            </AlertDialog>
            ...
            >
                <DialogContent className="!w-[95vw] max-w-5xl h-[92vh] rounded-[2rem] md:rounded-[3rem] border-neutral-100 dark:border-neutral-800 p-0 overflow-hidden bg-white dark:bg-black shadow-2xl">
                    {previewProduct && (() => {
                        const discount = activeDiscounts.find(d => d.id === previewProduct.discountId);
                        ...
                        const discountedGHS = calculateDiscountedPrice(previewProduct.price);
                                ...
                                <div className="md:col-span-7 p-6 md:p-10 lg:p-12 space-y-8 md:space-y-12 md:h-full md:overflow-y-auto">
                                    ...
                                    >
                                        <div className="flex flex-wrap items-center gap-3 mb-4 md:mb-6">
                                            <Badge variant="outline" className="text-[8px] md:text-[9px] font-black uppercase tracking-[0.2em] px-3 md:px-4 py-1.5 border-neutral-200 dark:border-neutral-800 bg-neutral-50/50 dark:bg-neutral-900/50">
                                            ...
                                            {previewProduct.isNewDrop && (
                                                <div className="flex items-center gap-2 bg-emerald-500/10 text-emerald-500 rounded-full pl-2.5 pr-3.5 py-1.5 border border-emerald-500/20">
                                                    ...
                                                    <span className="text-[8px] md:text-[9px] font-black uppercase tracking-widest">Active Drop</span>
                                                </div>
                                            )}
                                        </div>
                                        <h2 className="font-serif text-3xl md:text-4xl lg:text-5xl font-bold mb-3 md:mb-4 tracking-tight leading-tight uppercase break-words">{previewProduct.name}</h2>
                                        <div className="flex flex-wrap items-center gap-3 md:gap-4 mb-6 md:mb-8">
                                            <p className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.3em] text-neutral-400">ID: {previewProduct.id.slice(-8)}</p>
                                            <span className="hidden sm:inline w-1 h-1 rounded-full bg-neutral-200 dark:bg-neutral-800" />
                                            <p className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.3em] text-neutral-400 truncate">{previewProduct.collection?.name || "Independent Piece"}</p>
                                        </div>
                                        <p className="text-neutral-500 font-light leading-relaxed text-sm md:text-base max-w-xl italic">
                                            "{previewProduct.description || "No description provided for this architectural piece."}"
                                        </p>
                                    </motion.div>
                                    ...
                                    >
                                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-5">
                                            ...
                                            <div className="p-4 md:p-6 bg-neutral-50 dark:bg-neutral-900/50 rounded-2xl md:rounded-[1.5rem] border border-neutral-100 dark:border-neutral-900">
                                                <p className="text-[7px] md:text-[8px] font-black text-rose-500 uppercase tracking-[0.2em] mb-1.5 md:mb-2">Cost Price (GHS)</p>
                                                <p className="text-xl md:text-2xl font-black tracking-tighter">GH₵ {previewProduct.costPrice?.toLocaleString() || "---"}</p>
                                            </div>
                                            {/* Selling Price */}
                                            <div className="p-4 md:p-6 bg-black dark:bg-white text-white dark:text-black rounded-2xl md:rounded-[1.5rem] shadow-xl">
                                                <p className="text-[7px] md:text-[8px] font-black opacity-60 uppercase tracking-[0.2em] mb-1.5 md:mb-2">Selling Price (GHS)</p>
                                                <p className="text-xl md:text-2xl font-black tracking-tighter">GH₵ {previewProduct.price.toLocaleString()}</p>
                                            </div>
                                            {/* Discounted Price */}
                                            {discountedGHS && (
                                                <div className="p-4 md:p-6 bg-emerald-50 dark:bg-emerald-500/10 rounded-2xl md:rounded-[1.5rem] border border-emerald-500/20">
                                                    <p className="text-[7px] md:text-[8px] font-black text-emerald-500 uppercase tracking-[0.2em] mb-1.5 md:mb-2">Discount Price (GHS)</p>
                                                    <p className="text-xl md:text-2xl font-black tracking-tighter text-emerald-500">GH₵ {discountedGHS.toLocaleString()}</p>
                                                </div>
                                            )}
                                        </div>
                                    </motion.div>
                                    ...
                                    {previewProduct.prices?.length > 0 && (
                                        ...
                                        >
                                            {previewProduct.prices?.map((price: any) => (
                                                <div key={price.currencyCode} className="p-4 md:p-6 bg-neutral-50 dark:bg-neutral-900/50 rounded-2xl md:rounded-[1.5rem] border border-neutral-100 dark:border-neutral-900 group hover:bg-neutral-100/50 transition-colors">
                                                    <p className="text-[7px] md:text-[8px] font-black text-neutral-400 group-hover:text-neutral-500 transition-colors uppercase tracking-[0.2em] mb-1.5 md:mb-2">{price.currencyCode}</p>
                                                    <div className="flex flex-col">
                                                        <p className={`text-xl md:text-2xl font-black tracking-tighter ${price.discountPrice ? 'text-emerald-500' : 'text-black dark:text-white'}`}>
                                                            {price.discountPrice?.toLocaleString() || price.price.toLocaleString()}
                                                        </p>
                                                        {price.discountPrice && (
                                                            <span className="text-[9px] md:text-[10px] font-bold opacity-30 line-through mt-0.5 md:mt-1 tracking-tight">{price.price.toLocaleString()}</span>
                                                        )}
                                                    </div>
                                                </div>
                                            ))}
                                        ...
                                        >
                                            <div className="flex flex-col gap-2 md:gap-2.5">
                                                {previewProduct.variants?.map((v: any) => (
                                                    ...
                                                    >
                                                        <span className={`text-[9px] md:text-[10px] font-bold uppercase tracking-widest ${v.isAvailable ? 'text-neutral-700 dark:text-neutral-300' : 'text-rose-400'}`}>
                                                            {v.name}
                                                        </span>
                                                        {v.isAvailable ? (
                                                            <span className="text-[7px] md:text-[8px] font-black uppercase tracking-widest text-emerald-500 bg-emerald-500/10 px-2 py-0.5 rounded opacity-0 group-hover:opacity-100 transition-opacity">Ready</span>
                                                        ) : (
                                                            <span className="text-[7px] md:text-[8px] font-black uppercase tracking-widest">Out</span>
                                                        )}
                                                    </div>
                                                ))}
                                            </div>
                                            ...
                                            <div className="p-6 md:p-8 border border-dashed border-neutral-200 dark:border-neutral-800 rounded-2xl md:rounded-[2rem] flex flex-col items-center justify-center text-center gap-3 md:gap-4">
                                                ...
                                                <div>
                                                    <p className="text-[10px] md:text-[11px] font-black uppercase tracking-[0.1em] text-black dark:text-white mb-0.5 md:mb-1">
                                                        {previewProduct.relatedProducts?.length || 0} Recommendations
                                                    </p>
                                                    <p className="text-[8px] md:text-[9px] font-medium text-neutral-400 uppercase tracking-widest leading-relaxed">Linked for "Complete the Look"</p>
                                                </div>
                                            </div>
                                        </motion.div>
                                    </div>
                                    ...
                                    >
                                        <p className="text-[8px] md:text-[9px] font-black text-neutral-400 uppercase tracking-[0.3em]">Last Sync: {new Date().toLocaleDateString()}</p>
                                        <div className="flex gap-3 md:gap-4 w-full sm:w-auto">
                                            ...
                                            >
                                                <Link href={`/admin/products/upload?id=${previewProduct.id}`}>
                                                    Refine Archive
                                                </Link>
                                            </Button>
                                        </div>
