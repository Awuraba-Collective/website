    ...
    return (
        <div className="pt-12 border-t border-neutral-100 dark:border-neutral-900 space-y-8">
            <div className="flex items-center justify-between">
                <h2 className="text-xl font-bold text-black dark:text-white font-serif uppercase tracking-widest text-sm">Product Variants</h2>
                <Button
                    type="button"
                    variant="outline"
                    onClick={() => append({ id: Math.random().toString(36).substr(2, 9), name: "", available: true })}
            ...
            <div className="grid grid-cols-1 gap-4">
                                ...
                                <FormField
                                    ...
                                    render={({ field: toggleField }) => (
                                        <div className="flex items-center gap-3">
                                            <span className={`text-[9px] font-black uppercase tracking-widest transition-colors ${toggleField.value ? 'text-emerald-500' : 'text-neutral-400'}`}>
                ...
                {fields.length === 0 && (
                    <div className="text-center py-12 border border-dashed border-neutral-200 dark:border-neutral-800 rounded-xl bg-neutral-50/20">
                        <p className="text-[10px] text-neutral-400 uppercase font-black tracking-[0.2em]">No Active Variants</p>
                    </div>
                )}
                {/* Global error for variants array */}
                <FormMessage className="text-center">{control.getFieldState("variants").error?.message}</FormMessage>
            </div>
        </div>
    );
}
