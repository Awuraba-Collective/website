...
export const useProductForm = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [currRes, discRes, fitCatRes, catRes, collRes] =
          await Promise.all([
        ...
        if (productId) {
          ...
          if (prodRes.ok) {
            ...
            const ghsPrice = product.prices?.find(
                ...
                product.media?.map((img: any) => ({
                  ...
                  wearingVariant:
                    product.variants?.find(
                      (v: any) => v.name === img.modelWearingVariant,
                    )?.id || img.modelWearingVariant,
                  type: img.type || "IMAGE",
                })) || [],
              frequentlyBoughtTogether:
                product.relatedProducts?.map((rp: any) => rp.id) || [],
              newDrop: {
  ...
  const calculatePricing = useCallback(
    (pricingInput?: ProductFormValues["pricing"]) => {
      const { costPrice, projectedProfit, discountId, currencyOverrides } =
        pricingInput || form.getValues("pricing");

      // 1. Base Price (GHS)
      const priceGHS = costPrice + projectedProfit;
      const marginPercentage =
        costPrice > 0 ? (projectedProfit / costPrice) * 100 : 0;
    ...
    try {
      ...
      const payload: ProductApiPayload = {
        ...
        images: values.productImages.map((img, index) => {
          ...
          if (img.file instanceof File) {
            ...
            if (fileIndex !== -1 && uploadedUrls[fileIndex]) {
              return {
                ...
                wearingVariant:
                  values.variants.find((v) => v.id === img.wearingVariant)
                    ?.name ||
                  img.wearingVariant ||
                  undefined,
                type: img.type || "IMAGE",
          ...
          return {
            id: img?.id?.startsWith("new-") ? "" : img.id,
            url: img.previewUrl || "",
            alt: img.alt || `${values.name} - ${index + 1}`,
            modelHeight: img.modelHeight || undefined,
            wearingSize: img.wearingSize || undefined,
            wearingVariant:
              values.variants.find((v) => v.id === img.wearingVariant)?.name ||
              img.wearingVariant ||
              undefined,
            type: img.type || "IMAGE",
          };
        ...
        newDrop: values.newDrop?.enabled
          ? {
              ...
              expiresAt: new Date(
                new Date(originalCreatedAt || new Date()).getTime() +
                  (values.newDrop.durationDays || 14) * 24 * 60 * 60 * 1000,
              ).toISOString(),
            }
          : undefined,
      ...
      if (!res.ok) {
        ...
        throw new Error(
          err.error || `Failed to ${productId ? "update" : "publish"} product`,
        );
      }

      toast.success(
        `Product ${productId ? "updated" : "published"} successfully!`,
      );
      if (!productId) {
        form.reset();
      } else {
