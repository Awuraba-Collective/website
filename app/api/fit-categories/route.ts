...
export async function GET() {
    try {
        ...
        return NextResponse.json({ error: "Failed to fetch fit categories" }, { status: 500 });
    }
}
...
export async function POST(req: Request) {
    try {
        ...
        if (existing) {
            return NextResponse.json({ error: "Fit category already exists" }, { status: 409 });
        }
        ...
        return NextResponse.json({ error: "Failed to create fit category" }, { status: 500 });
    }
}
...
export async function DELETE(req: Request) {
    try {
        ...
        return NextResponse.json({ error: "Failed to delete fit category" }, { status: 500 });
    }
}
